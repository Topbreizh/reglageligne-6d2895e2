...
const ProduitFicheBloc = ({ bloc, produit, estChampVisible, getChampValeur }: ProduitFicheBlocProps) => {
  ...
  return (
    <div className="printable-block mb-2 p-1 border border-gray-200 rounded-lg print:p-0.25 print:text-xs print:border-[0.5px] print:mb-0.25 print:break-inside-avoid">
      <h2 className="text-sm font-bold mb-0.5 text-jaune-500 print:mb-0 print:text-[9px]">{bloc.nom}</h2>
      <div className="space-y-0">
        {champsVisibles
          .sort((a, b) => a.ordre - b.ordre)
          .map(champ => (
            <div key={champ.id} className="flex flex-row items-start gap-1 mb-0.5 print:mb-0 print:gap-0.5">
              <div className="font-semibold text-xs print:text-[8px] w-20 print:w-14 min-w-fit whitespace-nowrap">{champ.nom}:</div>
